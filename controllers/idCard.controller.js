...
const storage = multer.diskStorage({
    destination: function (req, file, callback) {
      callback(null, process.env.IMG_PATH_IDCARD);
    },
    filename: function (req, file, callback) {
      ...
      const filePath = path.join(process.env.IMG_PATH_IDCARD, uniqueFilename);
...
let nextImgId = 1;
const uploadIdCardController = async (req, res) => {
    try {
